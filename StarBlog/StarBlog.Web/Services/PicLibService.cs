...
public class PicLibService
{
    ...
    public static async Task<(Image, IImageFormat)> GenerateSizedImageAsyncOld(string imagePath, int width, int height)
    {
        ...
        var format = await Image.DetectFormatAsync(imagePath);
    ...
    public async Task<(Image, IImageFormat)> GetRandomImageAsync(string? seed = null)
    {
        ...
        //var (image, format) = await Image.LoadWithFormatAsync(fileStream);
        // 以上写法在3.0.2失效
        //var task1 = Task.Run(async () => await Image.LoadAsync(imagePath));
        //var task2 = Task.Run(async () => await Image.DetectFormatAsync(imagePath));
        Image image = await Image.LoadAsync(imagePath);
        IImageFormat format = await Image.DetectFormatAsync(imagePath);
        return (image, format);
    }
}